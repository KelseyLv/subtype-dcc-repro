...
class DCL(object):
    """
    Decoupled Contrastive Loss (https://arxiv.org/pdf/2110.06848.pdf)
    """
...
class ClusterLoss(nn.Module):
    ...
    def forward(self, c_i, c_j):
        ...
        p_i /= p_i.sum().clamp_min(1e-12)
        # xlogy: 0*log(0)->0; plain p*log(p) gives NaN when a cluster has zero mass (e.g. small cohorts / LIHC).
        ne_i = math.log(p_i.size(0)) + torch.xlogy(p_i, p_i).sum()
        p_j = c_j.sum(0).view(-1)
        p_j /= p_j.sum().clamp_min(1e-12)
        ne_j = math.log(p_j.size(0)) + torch.xlogy(p_j, p_j).sum()
        ne_loss = ne_i + ne_j
